...
export default function AnalyticsDashboard({
  ...
  const { totalAssets, totalLiabilities, netWorth } = useMemo(() => {
    // Assets
    const bankBalance = bankAccounts.reduce((acc, curr) => {
      let val = 0;
      if (curr.accountType === 'Crypto Wallet' && curr.cryptoBalances) {
        Object.entries(curr.cryptoBalances).forEach(([coin, amount]) => {
          if (coin === 'USDT' || coin === 'USD') val += amount * 85;
          else if (coin === 'BTC') val += amount * 5500000;
          else if (coin === 'ETH') val += amount * 250000;
          // Default ignore unknown coins for net worth safety
        });
      } else {
        val = Number(curr.currentBalance || 0);
        if (curr.currency === 'USD' || curr.currency === 'USDT') {
          val = val * 85; // Default exchange rate for net worth estimation
        }
      }
      return acc + val;
    }, 0);
    ...
    <div className="space-y-4 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <motion.div 
          ...
          className="bg-slate-900 p-4 rounded-lg text-white shadow-sm relative overflow-hidden"
        >
          <div className="absolute -right-4 -top-4 w-24 h-24 bg-indigo-500 rounded-full blur-3xl opacity-30"></div>
          <p className="text-xs font-bold text-slate-400 capitalize mb-1 flex items-center gap-1.5">
        ...
        <motion.div 
          ...
          className="bg-white border border-slate-150 p-4 rounded-lg shadow-sm"
        >
          <p className="text-xs font-bold text-slate-500 capitalize mb-1 flex items-center gap-1.5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <motion.div 
          ...
          className="bg-white border border-slate-150 p-4 rounded-lg shadow-sm"
        >
          <h3 className="font-bold text-slate-800 text-xs mb-4 flex items-center gap-1.5 capitalize">
        ...
        <motion.div 
          ...
          className="bg-white border border-slate-150 p-4 rounded-lg shadow-sm lg:col-span-2"
        >
          <h3 className="font-bold text-slate-800 text-xs mb-4 flex items-center gap-1.5 capitalize">
