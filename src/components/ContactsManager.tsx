...
import { getAccessToken, setAccessToken, signInWithGoogle, logout, db } from '../firebase';
...
export default function ContactsManager({
  ...
  const [isDueSubmitting, setIsDueSubmitting] = useState(false);
  ...
  const handleSaveDue = async (e: React.FormEvent) => {
    ...
    try {
      if (onAddPayment) {
        ...
        // Auto-create/sync LedgerProfile for Khata
        if (user && !user.uid.startsWith('guest_offline_')) {
           const q = query(collection(db, 'ledger_profiles'), where('userId', '==', user.uid), where('name', '==', selectedContactForDue.name));
           const snap = await getDocs(q);
           
           if (snap.empty) {
             const netBalance = dueType === 'owed' ? amt : -amt;
             const newProfile = {
                id: uuidv4(),
                userId: user.uid,
                name: selectedContactForDue.name,
                phone: selectedContactForDue.phone || '',
                netBalance: netBalance,
                createdAt: new Date().toISOString()
             };
             await setDoc(doc(db, 'ledger_profiles', newProfile.id), newProfile);
             
             // Create initial transaction in ledger
             const initialTx = {
                id: uuidv4(),
                ledgerId: newProfile.id,
                userId: user.uid,
                type: dueType === 'owed' ? 'gave' : 'got',
                amount: amt,
                date: new Date().toISOString().split('T')[0],
                memo: `From Contact: ${dueNotes}`,
                paymentMethod: 'cash',
                createdAt: new Date().toISOString()
             };
             await setDoc(doc(db, 'ledger_transactions', initialTx.id), initialTx);
           }
        }
  ...
  if (!isContactsLinked) {
    return (
      <div className="bg-white border border-slate-200/90 rounded-xl p-4 shadow-xs max-w-4xl mx-auto text-center space-y-2" id="contacts-manager-container">
        <div className="mx-auto p-2 bg-indigo-50 text-indigo-700 rounded-xl border border-indigo-100 w-16 h-16 flex items-center justify-center">
          <Users size={30} />
        </div>
  ...
  return (
    <div className="space-y-3 md:p-1" id="contacts-manager-container">
      {/* Simplified Header */}
      <div className="flex items-center justify-between bg-white rounded-lg p-3 border border-slate-100 shadow-sm mb-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-2" id="contacts-kpi-grid">
        <div className="bg-white rounded-lg border border-slate-100 p-2 shadow-xs" id="contacts-kpi-total">
          <p className="text-xs capitalize font-bold text-slate-500">Rolodex Capacity</p>
        ...
        <div className="bg-white rounded-lg border border-slate-100 p-2 shadow-xs" id="contacts-kpi-owed-to">
          <p className="text-xs capitalize font-bold text-emerald-600">Total Money Receivable</p>
          <div className="flex items-baseline gap-1 mt-1">
        ...
        <div className="bg-white rounded-lg border border-slate-100 p-2 shadow-xs" id="contacts-kpi-owed-by">
          <p className="text-xs capitalize font-bold text-rose-500">Total Money Payable</p>
          <div className="flex items-baseline gap-1 mt-1">
      ...
      <div className="bg-white rounded-lg p-2 md:p-3 border border-slate-100 shadow-sm" id="contacts-workspace-card">
        <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-2 pb-2 border-b border-slate-100">
          ...
          <div className="relative max-w-sm w-full" id="contacts-search-box">
            ...
            <input
              ...
              className="w-full text-xs pl-9 pr-2 py-1 borders border-slate-205 rounded-xl bg-slate-50 focus:bg-white focus:ring-1 focus:ring-slate-900/10 focus:border-slate-900 focus:outline-none transition-all outline-none"
            />
          </div>
        </div>
              ...
              {filteredContacts.map((contact, index) => {
                ...
                return (
                  <motion.div
                    ...
                    className="border border-slate-100 hover:border-slate-300 rounded-lg p-2 transition-all hover:shadow-md bg-white flex flex-col justify-between"
                  ...
                  >
                    <div>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <div>
                            ...
                            {contact.organization && (
                              <p className="text-xs text-slate-500 font-medium leading-none mt-1">{contact.organization}</p>
                            )}
                          </div>
                        </div>
                      ...
                      <div className="mt-2 space-y-1.5">
                        ...
                        {contact.email && (
                          <div className="flex items-center gap-1 text-xs text-slate-700 truncate">
                    ...
                    <div className="mt-2 pt-1 border-t border-slate-50 flex items-center justify-between text-xs font-mono">
                    ...
                    <button
                      ...
                      className="mt-2 w-full text-center text-xs font-bold text-slate-700 hover:text-slate-900 border border-slate-100 bg-slate-50 hover:bg-slate-100 py-1 rounded-md transition-colors"
                    >
                    ...
                    {expandedContact === contact.resourceName && (
                      <div className="mt-2 pt-2 border-t border-slate-100 space-y-1.5">
                        <h5 className="text-xs font-bold text-slate-500 capitalize ">History</h5>
                        {(() => {
                          const history = pendingPayments.filter(p => p.person.toLowerCase() === contact.name.toLowerCase());
                          if (history.length === 0) return <p className="text-xs text-slate-500">No transactions recorded.</p>;
                          
                          return history.map(h => (
                            <div key={h.id} className="flex items-center justify-between text-xs bg-white border border-slate-100 p-1 rounded">
                              <div className="flex items-center gap-1">
      ...
      {isFormOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-2" id="contact-form-modal">
          <motion.div
            ...
            className="bg-white rounded-xl p-3 md:p-4 max-w-md w-full shadow-2xl relative border border-slate-100"
          >
            <button
            ...
            <form onSubmit={handleSaveContact} className="mt-3 space-y-2">
              <div className="grid grid-cols-2 gap-1">
                <div>
                  <label className="block text-xs font-bold text-slate-500 capitalize mb-1">First (Given) Name *</label>
                  <input
                    type="text"
                    required
                    value={formGivenName}
                ...
                <div>
                  <label className="block text-xs font-bold text-slate-500 capitalize mb-1">Surname (Family Name)</label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 capitalize mb-1">Mobile / Phone Number</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 capitalize mb-1">Email Address</label>
                <input
                  type="email"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 capitalize mb-1">Company / Organization</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 capitalize mb-1">Associate Tag Classification</label>
                <select
                  value={formCategory}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-2" id="contact-due-modal">
          ...
          >
            ...
            <form onSubmit={handleSaveDue} className="mt-3 space-y-2">
              <div>
                <label className="block text-xs font-bold text-slate-500 capitalize mb-1">Due Type</label>
                <div className="grid grid-cols-2 gap-1">
                  <button
                    type="button"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 capitalize mb-1">Lump Sum Amount (INR) *</label>
                <input
                  type="number"
                  required
                  step="any"
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 capitalize mb-1">Payment Settlement Date</label>
                <input
                  type="date"
                  required
                  value={dueDate}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-500 capitalize mb-1">Notes / Terms</label>
                <textarea
                ...
                <button
                  type="button"
                  onClick={() => setIsDueFormOpen(false)}
                  disabled={isDueSubmitting}
                  className="px-3 py-1.5 text-slate-700 hover:text-slate-900 font-semibold text-xs rounded transition-colors"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  disabled={isDueSubmitting}
                  className="flex items-center gap-1 bg-slate-900 hover:bg-slate-950 text-white font-semibold text-xs px-4 py-1.5 rounded transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  <Save size={14} />
                  {isDueSubmitting ? 'Linking...' : 'Save & Link'}
                </button>
              </div>
              </div>
            </form>
