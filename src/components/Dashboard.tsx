...
export default function Dashboard({
  ...
  return (
    <div className="space-y-3" id="dashboard-tab">
      ...
      <div className="flex flex-row justify-between items-center gap-2 bg-white p-2 rounded-lg border border-slate-200/85">
        <div>
          <h2 className="text-xl font-extrabold text-slate-900 tracking-tight ">Dashboard</h2>
          <p className="text-xs font-bold text-slate-500 capitalize mt-0.5">Financial Year Overview • {monthLabel}</p>
        </div>
        
        <div className="flex items-center gap-1">
          <button 
            onClick={() => navigateMonth('prev')}
            className="px-1.5 py-1.5 text-xs font-bold text-slate-600 hover:text-slate-900 bg-white hover:bg-slate-50 border border-slate-200 rounded-lg cursor-pointer whitespace-nowrap"
          >
            ← Prev Month
          </button>
          <button 
      ...
      {(overdueCount > 0 || overdueCcBillsCount > 0) && (
        <div className="bg-red-50 border border-red-200/50 text-red-950 p-2 rounded-xl flex items-center justify-between gap-1 text-xs">
          ...
          <button 
            ...
            className="text-xs font-bold capitalize bg-slate-900 text-white hover:bg-slate-800 px-1 py-1.5 rounded-lg transition-colors cursor-pointer"
          >
            Settle Now
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
        ...
        <div className="bg-slate-900 text-white rounded-lg p-3 shadow-md border border-slate-800 flex flex-col justify-between space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 pt-2 border-t border-slate-800 text-xs">
            <div className="space-y-1">
              <span className="text-xs text-slate-500 font-bold">Total Assets</span>
              <p className="font-extrabold text-slate-100">₹{netWorthSummary.assets.toLocaleString('en-IN', { maximumFractionDigits: 0 })}</p>
            </div>
            <div className="space-y-1">
              <span className="text-xs text-slate-500 font-bold">Total Liabilities</span>
          ...
          <div className="pt-1 -ml-2 -mb-2">
            <button
              ...
              className="text-xs text-slate-500 hover:text-white active:text-white font-bold underline cursor-pointer px-2 py-2"
            >
        ...
        <div className="bg-white border border-slate-150 rounded-lg p-3 shadow-sm flex flex-col justify-between space-y-2">
          <div>
          ...
          <button
            ...
            className="w-full text-center py-1 bg-slate-100/80 hover:bg-slate-200 rounded-lg text-xs font-black text-slate-800 transition-colors cursor-pointer"
          >
            Investments Ledger →
          </button>
        </div>
      ...
      <AnimatePresence>
        {showPreferences && (
          <motion.div
            ...
            className="overflow-hidden bg-slate-50 p-2 rounded-lg border border-slate-150 grid grid-cols-1 md:grid-cols-5 gap-2 text-xs "
          >
            <div className="col-span-1 md:col-span-5 border-b border-slate-200/50 pb-1 flex justify-between items-end">
              <div>
                ...
                <p className="text-xs text-slate-500">Ledger Cash (₹{lifetimeLedgerCash.toLocaleString()}) is already auto-included in Net Worth.</p>
              </div>
            </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-2 bg-slate-50 border border-slate-150 p-2 rounded-lg text-xs text-slate-800">
        ...
        <div className="flex items-center gap-1.5 p-1">
          ...
          <div>
            <span className="text-xs text-slate-500 font-bold capitalize">Pending Collections</span>
        ...
        <div className="flex items-center gap-1.5 p-1">
          ...
          <div>
            ...
            <p className="font-extrabold text-[13px]">{activeEmis.length} running <span className="text-xs font-mono text-slate-500">(-₹{totalMonthlyEmi}/mo)</span></p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-3">
        ...
        <div className="lg:col-span-3 bg-white p-3 rounded-lg border border-slate-150 shadow-sm space-y-2">
        ...
        <div className="lg:col-span-2 bg-white p-3 rounded-lg border border-slate-150 shadow-sm flex flex-col justify-between space-y-2">
          <div>
            <div className="flex justify-between items-center mb-2">
              <h3 className="font-bold text-sm text-slate-800 font-display">Recent Activity</h3>
              <button 
                onClick={() => onNavigateToTab('transactions')}
                className="text-xs font-bold text-slate-450 hover:text-slate-900 border border-slate-100 hover:border-slate-200 rounded-md p-1 px-1.5 transition-colors cursor-pointer"
              >
                All ledger
              </button>
            </div>
            ...
            {recentTransactions.length === 0 ? (
              <p className="text-xs text-slate-500 py-3 text-center">Your transactions ledger is currently empty.</p>
            ) : (
          ...
          <div className="border-t border-slate-100 pt-1">
            <span className="text-xs text-slate-500 font-semibold capitalize ">Audit Control Assurance</span>
          </div>
        </div>
