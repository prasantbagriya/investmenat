...
export default function TaskCard({ 
  ...
  const isSyncedToCalendar = task.syncedToCalendar === true;
  ...
  return (
    <motion.div
      ...
      whileHover={{ y: -2 }}
      className={`relative overflow-hidden rounded-lg border transition-all p-2 flex flex-col md:flex-row md:items-center justify-between gap-2 shadow-xs ${
        !isPending
    ...
    >
      ...
      <div className="space-y-1 flex-1">
        <div className="flex items-center gap-1">
          {!isPending ? (
            <span className="inline-flex items-center gap-1 bg-emerald-50 border border-emerald-200 text-emerald-700 text-xs font-bold px-1 py-0.5 rounded-full capitalize ">
              <Check className="w-2.5 h-2.5" /> Completed
            </span>
          ) : isOverdue ? (
            <span className="inline-flex items-center gap-1 bg-rose-50 border border-rose-200 text-rose-700 text-xs font-bold px-1 py-0.5 rounded-full capitalize animate-pulse">
              <AlertTriangle className="w-2.5 h-2.5" /> ⚠️ Overdue
            </span>
          ) : (
            <span className="inline-flex items-center gap-1 bg-slate-50 border border-slate-200 text-slate-700 text-xs font-bold px-1 py-0.5 rounded-full capitalize ">
          ...
          {/* Overdue Badge */}
          {isPending && isOverdue && (
            <span className="text-xs font-bold font-mono text-rose-600 bg-rose-100 px-1 py-0.5 rounded-md">
      ...
      <div className="flex items-center gap-2 border-t border-slate-100 pt-1 md:pt-0 md:border-t-0 justify-between md:justify-end">
        ...
        {isPending && (
          <div className="text-right flex flex-col justify-center">
            <span className="text-xs capitalize font-bold text-slate-500 ">Time Remaining</span>
        ...
        <div className="flex items-center gap-1">
          {isCalendarLinked && isPending && (
            isSyncedToCalendar ? (
              <span className="inline-flex items-center gap-1 bg-rose-55 text-rose-700 text-xs font-bold px-1 py-1.5 rounded-xl border border-rose-100">
