...
export function WealthForecaster({ sips, fds, holdings, physicalAssets = [], bankAccounts = [], livePrices }: WealthForecasterProps) {
  ...
  const forecastData = useMemo(() => {
    ...
    const currentBank = bankAccounts.reduce((sum, b) => sum + Number(b.currentBalance || 0), 0);
  ...
  return (
    <div className="space-y-6 pb-24 max-w-lg mx-auto">
      <div className="bg-linear-to-r from-indigo-900 to-indigo-800 rounded-xl p-6 text-white shadow-xl relative overflow-hidden">
        <div className="absolute top-0 right-0 p-4 opacity-10">
          <TrendingUp size={120} />
        </div>
        
        <div className="relative z-10">
          <h2 className="text-indigo-200 font-medium text-sm mb-1">PROJECTED WEALTH IN {projectionYears} YEARS</h2>
