...
import { restoreFromCloud, initCloudSyncInterceptor, resetCloudSyncInterceptor } from './utils/cloudSync';
import { mergeWithFallback } from './utils/fallbackManager';
import { useFallbackSync } from './hooks/useFallbackSync';
...
import {
  Holding, Sip, Fd, WatchlistItem, UserSettings, RealizedTrade, RecurringBill, BankAccount, CreditCardBill, EmiItem, PropFirmAccount, PropFirmPayout, PropFirmChallenge,
...
const CryptoWalletView = React.lazy(() => import('./components/CryptoWalletView'));
const PanVerification = React.lazy(() => import('./components/QuickoWorkspace/PanVerification'));
const BankVerification = React.lazy(() => import('./components/QuickoWorkspace/BankVerification'));
const AadhaarVerification = React.lazy(() => import('./components/QuickoWorkspace/AadhaarVerification'));
const GstinSearch = React.lazy(() => import('./components/QuickoWorkspace/GstinSearch'));
const BrokerageParser = React.lazy(() => import('./components/QuickoWorkspace/BrokerageParser'));
const QuickoConnect = React.lazy(() => import('./components/QuickoWorkspace/QuickoConnect'));
const QuickoCallback = React.lazy(() => import('./components/QuickoWorkspace/QuickoCallback'));
import { exportFullLedgerToCSV } from './utils/csvExport';
import { useTaskReminder } from './utils/useTaskReminder';

const PropFirmDashboard = React.lazy(() => import('./components/propfirm/PropFirmDashboard.tsx'));
const PropFirmChallenges = React.lazy(() => import('./components/propfirm/PropFirmChallenges.tsx'));
const PropFirmAccounts = React.lazy(() => import('./components/propfirm/PropFirmAccounts.tsx'));
const PropFirmPayouts = React.lazy(() => import('./components/propfirm/PropFirmPayouts.tsx'));

const KlineWorkspace = React.lazy(() => import('./components/kline/KlineWorkspace'));
...
export default function App() {
  ...
  // Background fallback sync worker
  useFallbackSync(user);
  ...
  const [propFirmPayouts, setPropFirmPayouts] = useState<PropFirmPayout[]>([]);
  const [propFirmChallenges, setPropFirmChallenges] = useState<PropFirmChallenge[]>([]);

  const uniqueFirmNames = useMemo(() => Array.from(new Set([
    ...propFirmChallenges.map(c => c.firmName),
    ...propFirmAccounts.map(a => a.firmName),
    ...propFirmPayouts.map(p => p.firmName)
  ])).filter(Boolean).sort(), [propFirmChallenges, propFirmAccounts, propFirmPayouts]);

  // Track if we've already applied the initial PIN lock so it doesn't randomly lock on reconnections
  const hasAppliedInitialLock = useRef(false);
  ...
  const [currentWorkspace, setCurrentWorkspace] = useState<'ledger' | 'investmant' | 'research' | 'crypto' | 'quicko' | 'propfirm'>('ledger');

  // Tab & month controller
  const navigate = useNavigate();
  const location = useLocation();
  const activeTab = location.pathname.split('/')[1] || (currentWorkspace === 'ledger' ? 'dashboard' : currentWorkspace === 'research' ? 'market-data' : currentWorkspace === 'crypto' ? 'crypto-wallet' : currentWorkspace === 'quicko' ? 'quicko-kyc' : currentWorkspace === 'propfirm' ? 'propfirm-dashboard' : 'portfolio');
  ...
  const handleApplyLoginManualToken = () => {
    ...
    handleGuestSignIn();
    toast.success("✅ Manual Access Token applied! Dynamic syncing is now enabled for your Session.");
  };
    ...
    const handleHashAndToken = () => {
      ...
      if (hash && hash.includes('access_token=')) {
        ...
        if (token) {
          ...
          // Clean URL parameters immediately but PRESERVE the hash for HashRouter!
  ...
  useEffect(() => {
    ...
    const processNotificationAction = async () => {
      ...
      // Clean action from hash by resetting router to root, avoiding React Router crash
  ...
  // Auto Logout timer - 30 minutes of complete inactivity
  useEffect(() => {
    if (!user) return;
    ...
    const resetInactivityTimer = () => {
      ...
      logoutTimer = setTimeout(() => {
        if (userSettings?.pin) {
          setIsLocked(true);
          hasAppliedInitialLock.current = false; // Reset lock tracking
          sessionStorage.removeItem('investmant_unlocked');
        }
      }, 30 * 60 * 1000); // 30 mins
    };
  ...
  useEffect(() => {
    ...
    window.addEventListener('open-nav-drawer', handleOpenNav);
    return () => {
      window.removeEventListener('trigger-google-sign-in', handleGoogleTrigger);
      window.removeEventListener('open-nav-drawer', handleOpenNav);
    };
  }, []);

  useEffect(() => {
    // NOTE: We intentionally do NOT bypass Firebase auth for offline mode.
    // isOfflineMode only controls the UI experience. Firebase auth runs always.
    const isOfflineUI = localStorage.getItem('isOfflineMode') === 'true';
    if (isOfflineUI) {
      ...
      // Still subscribe to Firebase auth in background for auto-migration
      // (so if Firebase user exists, we can offer migration)
    }
    ...
    const unsubscribe = onAuthStateChanged(auth, async (usr) => {
      ...
      if (usr && !usr.uid.startsWith('guest_offline_')) {
        ...
        // ✅ Activate cloud sync interceptor for this logged-in user
        initCloudSyncInterceptor(usr.uid);
        // ✅ Restore any cloud-saved localStorage data
        restoreFromCloud(usr.uid).catch(console.error);
      }
  ...
  useEffect(() => {
    ...
    if (user.uid.startsWith('guest_offline_')) {
      const loadLocalData = () => {
        ...
        setPropFirmChallenges(JSON.parse(localStorage.getItem(`propFirmChallenges_${user.uid}`) || '[]'));
        setPropFirmAccounts(JSON.parse(localStorage.getItem(`propFirmAccounts_${user.uid}`) || '[]'));
        setPropFirmPayouts(JSON.parse(localStorage.getItem(`propFirmPayouts_${user.uid}`) || '[]'));
        ...
        if (savedSettings.pin && sessionStorage.getItem('investmant_unlocked') !== 'true') {
          setIsLocked(true);
        } else {
          setIsLocked(false);
          if (savedSettings.pin) sessionStorage.setItem('investmant_unlocked', 'true');
        }
      };
      loadLocalData();
      return () => { };
    ...
    const unsubscribeSettings = onSnapshot(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          ...
          if (sets.pin && !hasAppliedInitialLock.current) {
            if (sessionStorage.getItem('investmant_unlocked') !== 'true') {
              setIsLocked(true); // Secure Lock is on initially
            }
            hasAppliedInitialLock.current = true;
          }
        } else {
    ...
    const unsubscribeTx = onSnapshot(
      ...
      (snapshot) => {
        ...
        setTransactions(mergeWithFallback(user, 'tx', txs));
      },
    ...
    const unsubscribeBankAccounts = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((d) => banks.push({ id: d.id, ...d.data() } as BankAccount));
        setBankAccounts(mergeWithFallback(user, 'bankAccounts', banks));
      },
    ...
    const unsubscribeBills = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((d) => bills.push({ id: d.id, ...d.data() } as CreditCardBill));
        setCcBills(mergeWithFallback(user, 'ccbills', bills));
      },
    ...
    const unsubscribeEmis = onSnapshot(
      ...
      (snapshot) => {
        const items: EmiItem[] = [];
        snapshot.forEach((d) => items.push({ id: d.id, ...d.data() } as EmiItem));
        setCcEmis(mergeWithFallback(user, 'ccemis', items));
      },
    ...
    const unsubscribePay = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((d) => p.push({ id: d.id, ...d.data() } as PendingPayment));
        setPendingPayments(mergeWithFallback(user, 'pay', p));
      },
    ...
    const unsubscribeGoals = onSnapshot(
      ...
      (snapshot) => {
        const g: SavingsGoal[] = [];
        snapshot.forEach((d) => g.push({ id: d.id, ...d.data() } as SavingsGoal));
        setSavingsGoals(mergeWithFallback(user, 'goals', g));
      },
    ...
    const unsubscribeLimits = onSnapshot(
      ...
      (snapshot) => {
        const l: BudgetLimit[] = [];
        snapshot.forEach((d) => l.push({ id: d.id, ...d.data() } as BudgetLimit));
        setBudgetLimits(mergeWithFallback(user, 'limits', l));
      },
    ...
    const unsubscribeHoldings = onSnapshot(
      ...
      (snapshot) => {
        const h: Holding[] = [];
        snapshot.forEach((d) => h.push({ id: d.id, ...d.data() } as Holding));
        setHoldings(mergeWithFallback(user, 'holdings', h));
      },
    ...
    const unsubscribeSips = onSnapshot(
      ...
      (snapshot) => {
        const s: Sip[] = [];
        snapshot.forEach((d) => s.push({ id: d.id, ...d.data() } as Sip));
        setSips(mergeWithFallback(user, 'sips', s));
      },
    ...
    const unsubscribeFds = onSnapshot(
      ...
      (snapshot) => {
        const f: Fd[] = [];
        snapshot.forEach((d) => f.push({ id: d.id, ...d.data() } as Fd));
        setFds(mergeWithFallback(user, 'fds', f));
      },
    ...
    const unsubscribeWatchlist = onSnapshot(
      ...
      (snapshot) => {
        ...
        snapshot.forEach((d) => w.push({ id: d.id, ...d.data() } as WatchlistItem));
        setWatchlist(mergeWithFallback(user, 'watchlist', w));
      },
    ...
    const unsubscribeRealized = onSnapshot(
      ...
      (snapshot) => {
        ...
        setRealizedTrades(mergeWithFallback(user, 'realized_trades', rt));
      },
      (error) => handleFirestoreError(error, OperationType.LIST, 'realizedTrades')
    );

    // Prop Firm
    const propFirmAccQuery = query(collection(db, 'propFirmAccounts'), where('userId', '==', user.uid));
    const unsubscribePropFirmAcc = onSnapshot(
      propFirmAccQuery,
      (snapshot) => {
        const accs: PropFirmAccount[] = [];
        snapshot.forEach((d) => accs.push({ id: d.id, ...d.data() } as PropFirmAccount));
        setPropFirmAccounts(mergeWithFallback(user, 'propFirmAccounts', accs));
      },
      (error) => handleFirestoreError(error, OperationType.LIST, 'propFirmAccounts')
    );

    const propFirmPayoutQuery = query(collection(db, 'propFirmPayouts'), where('userId', '==', user.uid));
    const unsubscribePropFirmPayouts = onSnapshot(
      propFirmPayoutQuery,
      (snapshot) => {
        const payouts: PropFirmPayout[] = [];
        snapshot.forEach((d) => payouts.push({ id: d.id, ...d.data() } as PropFirmPayout));
        setPropFirmPayouts(mergeWithFallback(user, 'propFirmPayouts', payouts));
      },
      (error) => handleFirestoreError(error, OperationType.LIST, 'propFirmPayouts')
    );

    const propFirmChallengeQuery = query(collection(db, 'propFirmChallenges'), where('userId', '==', user.uid));
    const unsubscribePropFirmChallenges = onSnapshot(
      propFirmChallengeQuery,
      (snapshot) => {
        const challenges: PropFirmChallenge[] = [];
        snapshot.forEach((d) => challenges.push({ id: d.id, ...d.data() } as PropFirmChallenge));
        setPropFirmChallenges(mergeWithFallback(user, 'propFirmChallenges', challenges));
      },
      (error) => handleFirestoreError(error, OperationType.LIST, 'propFirmChallenges')
    );
    ...
    return () => {
      ...
      unsubscribeBills();
      unsubscribeEmis();
      unsubscribePropFirmAcc();
      unsubscribePropFirmPayouts();
      unsubscribePropFirmChallenges();
    };
  }, [user]);
  ...
  const handleAddTransaction = async (txData: Omit<Transaction, 'id' | 'userId'> & { id?: string }) => {
    ...
    if (txData.bankAccountId) {
      ...
      if (bank) {
        ...
        try {
          if (bank.accountType === 'Crypto Wallet') {
            const cur = txData.currency || bank.currency || 'USDT';
            const cb = { ...(bank.cryptoBalances || {}) };
            cb[cur] = (cb[cur] || 0) + diff;
            await handleEditBankAccount(bank.id, { cryptoBalances: cb });
          } else {
            await handleEditBankAccount(bank.id, { currentBalance: Number(bank.currentBalance || 0) + diff });
          }
        } catch (err) {
    ...
    if (txData.toBankAccountId && txData.type === 'transfer') {
      ...
      if (toBank) {
        try {
          if (toBank.accountType === 'Crypto Wallet') {
            const cur = txData.currency || toBank.currency || 'USDT';
            const cb = { ...(toBank.cryptoBalances || {}) };
            cb[cur] = (cb[cur] || 0) + txData.amount;
            await handleEditBankAccount(toBank.id, { cryptoBalances: cb });
          } else {
            await handleEditBankAccount(toBank.id, { currentBalance: Number(toBank.currentBalance || 0) + txData.amount });
          }
        } catch (err) {
  ...
  const handleEditTransaction = async (id: string, txData: Partial<Transaction>) => {
    ...
    if (oldTx) {
      // Create a helper to apply a diff to a bank, handling crypto properly
      const applyDiff = async (bankId: string, diff: number, currency: string | undefined) => {
        if (diff === 0) return;
        const bank = bankAccounts.find(b => b.id === bankId);
        if (!bank) return;
        try {
          if (bank.accountType === 'Crypto Wallet') {
            const cur = currency || bank.currency || 'USDT';
            const cb = { ...(bank.cryptoBalances || {}) };
            cb[cur] = (cb[cur] || 0) + diff;
            // Immediate update to state context to prevent race conditions during sequential awaits
            bank.cryptoBalances = cb; 
            await handleEditBankAccount(bank.id, { cryptoBalances: cb });
          } else {
            bank.currentBalance = Number(bank.currentBalance || 0) + diff;
            await handleEditBankAccount(bank.id, { currentBalance: bank.currentBalance });
          }
        } catch (err) {
      ...
      if (oldTx.bankAccountId) {
        ...
        await applyDiff(oldTx.bankAccountId, diff, oldTx.currency);
      }
      if (oldTx.toBankAccountId && oldTx.type === 'transfer') {
        await applyDiff(oldTx.toBankAccountId, -oldTx.amount, oldTx.currency);
      }

      // Apply new
      const newType = txData.type ?? oldTx.type;
      const newAmount = txData.amount ?? oldTx.amount;
      const newCurrency = txData.currency !== undefined ? txData.currency : oldTx.currency;
      ...
      if (newBankId) {
        ...
        await applyDiff(newBankId, diff, newCurrency);
      }
      if (newToBankId && newType === 'transfer') {
        await applyDiff(newToBankId, newAmount, newCurrency);
      }
    }
    ...
    if (tx) {
      if (tx.bankAccountId) {
        ...
        if (bank) {
          ...
          try {
            if (bank.accountType === 'Crypto Wallet') {
              const cur = tx.currency || bank.currency || 'USDT';
              const cb = { ...(bank.cryptoBalances || {}) };
              cb[cur] = (cb[cur] || 0) + diff;
              await handleEditBankAccount(bank.id, { cryptoBalances: cb });
            } else {
      ...
      if (tx.toBankAccountId && tx.type === 'transfer') {
        ...
        if (toBank) {
          try {
            if (toBank.accountType === 'Crypto Wallet') {
              const cur = tx.currency || toBank.currency || 'USDT';
              const cb = { ...(toBank.cryptoBalances || {}) };
              cb[cur] = (cb[cur] || 0) - tx.amount;
              await handleEditBankAccount(toBank.id, { cryptoBalances: cb });
            } else {
              await handleEditBankAccount(toBank.id, { currentBalance: Number(toBank.currentBalance || 0) - tx.amount });
            }
          } catch (err) {
            console.warn("Failed to update destination bank balance", err);
          }
  ...
  const handleSavePin = async () => {
    ...
    if (newPin.length !== 4 || isNaN(parseInt(newPin))) {
      toast.error('PIN must be 4 digits numeric.');
      return;
    }
    if (newPin !== confirmPin) {
      toast.error('PIN confirms mismatch.');
      return;
    }
    const hashedPin = await hashPIN(newPin);
    if (user.uid.startsWith('guest_offline_')) {
      const updated = { ...userSettings, pin: hashedPin } as UserSettings;
      setUserSettings(updated);
      localStorage.setItem(`settings_${user.uid}`, JSON.stringify(updated));
    ...
    const docRef = doc(db, 'usersettings', user.uid);
    await setDoc(docRef, { id: user.uid, pin: hashedPin }, { merge: true });
    setPinSetupActive(false);
    setNewPin('');
    setConfirmPin('');
    toast.success('Security PIN configured successfully!');
  ...
  const handleVerifyUnlock = async () => {
    if (userSettings && userSettings.pin) {
      const hashedInput = await hashPIN(pinValue);
      if (pinValue === userSettings.pin || hashedInput === userSettings.pin) {
        // Auto-migrate plaintext PIN to hashed PIN
        if (pinValue === userSettings.pin && !user.uid.startsWith('guest_offline_')) {
           setDoc(doc(db, 'usersettings', user.uid), { id: user.uid, pin: hashedInput }, { merge: true });
        }
        setIsLocked(false);
        setPinValue('');
        sessionStorage.setItem('investmant_unlocked', 'true');
      } else {
        toast.error('Oops, security authorization invalid. Retry PIN entry.');
        setPinValue('');
      }
    }
  };
  ...
  const handleNumpadPress = (num: string) => {
    if (pinValue.length < 4) {
      ...
      if (nextPin.length === 4) {
        setTimeout(async () => {
          if (userSettings) {
            const hashedInput = await hashPIN(nextPin);
            if (nextPin === userSettings.pin || hashedInput === userSettings.pin) {
              if (nextPin === userSettings.pin && !user?.uid.startsWith('guest_offline_')) {
                 setDoc(doc(db, 'usersettings', user?.uid!), { id: user?.uid, pin: hashedInput }, { merge: true });
              }
              setIsLocked(false);
              setPinValue('');
  ...
  const handleGoogleSignIn = () => {
    signInWithGoogle().catch((err) => {
      ...
      if (errorMsg.includes('unauthorized-domain')) {
        toast.error('Domain Not Authorized: Please add this website URL to Firebase Console -> Authentication -> Settings -> Authorized Domains.');
      } else {
  ...
  const handleMigrateGuestData = async () => {
    ...
    try {
      ...
      toast.success('Congratulations! All your guest data has been successfully migrated to your Google Account. Offline mode deactivated and secured in Cloud Firestore.');
    } catch (e: any) {
      console.error(e);
      setMigrationProgress('');
      toast.error('Error during data migration: ' + e.message);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col overflow-x-hidden selection:bg-indigo-500/30">
      <Toaster position="top-right" toastOptions={{ className: 'text-sm font-bold', style: { borderRadius: '12px', background: '#334155', color: '#fff' } }} />

      {/* Top Navigation */}
      {activeTab !== 'market' && activeTab !== 'kline-test' && (
        <Header
          user={user}
      ...
      <AnimatePresence>
        {pinSetupActive && (
          ...
          >
            <div className="max-w-xs mx-auto flex flex-col gap-3 text-xs ">
              <div className="text-center">
                <h4 className="font-extrabold text-slate-800 flex items-center justify-center gap-1 text-sm">
                  <Lock size={14} className="text-indigo-600" />
                  {userSettings?.pin ? 'Modify Security Lock PIN' : 'Initialize App Protection PIN'}
                </h4>
                <p className="text-xs text-slate-500 mt-0.5">Protect your investment sheets. Input exactly 4 digits.</p>
              </div>

              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-slate-700 font-bold text-xs mb-1">Choose 4-digit pin</label>
                  <input
                    type="password"
                    maxLength={4}
                    placeholder="••••"
                    ...
                    className="w-full bg-white border border-slate-200 rounded-lg p-1.5 font-bold text-center"
                  />
                </div>

                <div>
                  <label className="block text-slate-700 font-bold text-xs mb-1">Confirm pin</label>
                  <input
                    type="password"
                    maxLength={4}
                    placeholder="••••"
              ...
              <div className="flex justify-end gap-2 border-t border-slate-100 pt-2 mt-1">
                <button
                  ...
                  className="px-2.5 py-1.5 border border-slate-205 text-slate-700 rounded-lg text-xs font-bold cursor-pointer hover:bg-slate-50 transition-colors"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="px-2.5 py-1.5 bg-slate-900 text-white rounded-lg text-xs font-bold cursor-pointer hover:bg-slate-800 transition-colors"
                >
                  Save PIN
                </button>
              </div>
      ...
      <main className={`w-full grow ${(location.pathname === '/market' || location.pathname === '/kline-test') ? '' : 'pb-16 md:pb-0 max-w-8xl mx-auto px-2 sm:px-3 lg:px-4 mt-3'}`}>
        <div className="transition-all duration-300 h-full">
          <ErrorBoundary>
            <React.Suspense key={location.pathname} fallback={<div className="flex h-64 items-center justify-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-emerald-500"></div></div>}>
              <Routes>
                <Route path="/kline-test" element={<KlineWorkspace />} />

                {/* Quicko OAuth Callback Route */}
                <Route path="/quicko-callback" element={
                  <React.Suspense fallback={<div className="flex h-screen items-center justify-center bg-gray-900 text-white"><div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-emerald-500"></div></div>}>
                    <QuickoCallback />
                  </React.Suspense>
                } />

                <Route path="/" element={<Navigate to={currentWorkspace === 'ledger' ? '/dashboard' : currentWorkspace === 'research' ? '/market-data' : currentWorkspace === 'crypto' ? '/crypto-wallet' : currentWorkspace === 'quicko' ? '/quicko-kyc' : '/portfolio'} replace />} />
                <Route path="/crypto-wallet" element={<CryptoWalletView activeTab="dashboard" />} />
                <Route path="/crypto-send" element={<CryptoWalletView activeTab="send" />} />
                <Route path="/crypto-receive" element={<CryptoWalletView activeTab="receive" />} />
                <Route path="/crypto-security" element={<CryptoWalletView activeTab="security" />} />
                <Route path="/quicko-kyc" element={<div className="p-2 sm:p-3 pb-24"><PanVerification /></div>} />
                <Route path="/quicko-bank" element={<div className="p-2 sm:p-3 pb-24"><BankVerification /></div>} />
                <Route path="/quicko-aadhaar" element={<div className="p-2 sm:p-3 pb-24"><AadhaarVerification /></div>} />
                <Route path="/quicko-gstin" element={<div className="p-2 sm:p-3 pb-24"><GstinSearch /></div>} />
                <Route path="/quicko-brokerage" element={<div className="p-2 sm:p-3 pb-24"><BrokerageParser /></div>} />
                <Route path="/quicko-connect" element={<div className="p-2 sm:p-3 pb-24"><QuickoConnect /></div>} />
                ...
                <Route path="/market" element={<ResearchTerminal livePrices={livePrices} userSettings={userSettings} onUpdateSettings={handleUpdateSmartApiSettings} />} />
                ...
                <Route path="/tax" element={<TaxCapitalGains
                  ...
                  livePrices={livePrices}
                  userSettings={userSettings}
                  onUpdateSettings={handleUpdateSmartApiSettings}
                />} />
                ...
                <Route path="/propfirm-dashboard" element={<PropFirmDashboard accounts={propFirmAccounts} payouts={propFirmPayouts} challenges={propFirmChallenges} />} />
                <Route path="/propfirm-challenges" element={<PropFirmChallenges user={user} challenges={propFirmChallenges} setChallenges={setPropFirmChallenges} accounts={propFirmAccounts} setAccounts={setPropFirmAccounts} firmNames={uniqueFirmNames} />} />
                <Route path="/propfirm-accounts" element={<PropFirmAccounts user={user} accounts={propFirmAccounts} setAccounts={setPropFirmAccounts} firmNames={uniqueFirmNames} payouts={propFirmPayouts} />} />
                <Route path="/propfirm-payouts" element={<PropFirmPayouts user={user} payouts={propFirmPayouts} accounts={propFirmAccounts} setPayouts={setPropFirmPayouts} setAccounts={setPropFirmAccounts} bankAccounts={bankAccounts} onAddTransaction={handleAddTransaction} firmNames={uniqueFirmNames} />} />
        ...
        <AnimatePresence>
          {activeToasts.map((toast) => (
            <motion.div
              ...
              className="bg-slate-900 text-white rounded-lg p-2 shadow-xl border border-slate-700 pointer-events-auto flex gap-1.5 items-start justify-between"
            >
              ...
              <div className="flex flex-col gap-1 ml-2">
                <button
                  ...
                  className="px-2 py-1 text-xs font-bold bg-indigo-600/20 text-indigo-300 hover:bg-indigo-600/40 rounded transition-colors cursor-pointer"
                >
                  Snooze
                </button>
                <button
      ...
      <AnimatePresence>
        {snoozingTask && (
          ...
          >
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} exit={{ scale: 0.95 }} className="bg-white rounded-lg p-4 w-full max-w-xs shadow-2xl space-y-3">
              ...
              <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 mt-2">
                ...
                <button
                  onClick={async () => {
                    if (!snoozeDate || !snoozeTime) return toast.error("Please select date and time.");
                    try {
                    ...
                    } catch (e) { toast.error("Failed to snooze: " + e); }
                  }}
