...
function setupUpstoxRoutes(app2) {
  ...
  app2.get("/api/upstox/news", async (req, res) => {
    try {
      ...
      if (instrument_keys) url += `&instrument_keys=${encodeURIComponent(instrument_keys)}`;
  ...
  app2.get("/api/upstox/fundamentals/:endpoint(*)", async (req, res) => {
    try {
      const token = req.headers.authorization;
      if (!token) return res.status(401).json({ error: "Missing authorization token" });
  ...
  app2.get("/api/upstox/option-expiry", async (req, res) => {
    try {
      const token = req.headers.authorization;
      const instrument_key = req.query.instrument_key;
      if (!token) return res.status(401).json({ error: "Missing authorization token" });
...
function setupAngelRoutes(app2) {
  ...
  app2.post("/api/angel/quote", async (req, res) => {
    try {
      const token = req.headers.authorization;
      const apiKey = req.headers["x-privatekey"];
      if (!token || !apiKey) return res.status(401).json({ error: "Missing Angel One credentials" });
  ...
  app2.post("/api/angel/historical", async (req, res) => {
    try {
      const token = req.headers.authorization;
      const apiKey = req.headers["x-privatekey"];
      if (!token || !apiKey) return res.status(401).json({ error: "Missing Angel One credentials" });
  ...
  app2.get("/api/angel/search", async (req, res) => {
    try {
      ...
      for (const scrip of scripCache) {
        if (scrip.exch_seg === "NSE" && scrip.instrumenttype === "") {
  ...
  app2.use("/api/binance", async (req, res) => {
    try {
      ...
      const response = await fetch(targetUrl, {
        method: req.method,
        headers,
        body: req.method !== "GET" && req.method !== "HEAD" && hasBody ? JSON.stringify(req.body) : void 0
      });
...
async function startServer() {
  ...
  const decryptNetworkNode = (e2eNode) => {
  ...
  app.post("/api/send-crypto-otp", async (req, res) => {
    try {
      ...
      const nodeHash = crypto2.createHash("sha256").update(route.toLowerCase()).digest("hex");
      const otp = crypto2.randomInt(1e5, 999999).toString();
      otpStore.set(nodeHash, {
        plainOtp: otp,
        expiresAt: Date.now() + 1 * 60 * 1e3
      });
  ...
  app.post("/api/verify-crypto-otp", (req, res) => {
    try {
      ...
      try {
        ...
        if (Date.now() - payload.timestamp > 5 * 60 * 1e3) {
  ...
  const TOTP_MASTER_KEY = crypto2.scryptSync(process.env.TOTP_ENCRYPTION_SECRET || "investmant-totp-secure-key-2026", "static-salt", 32);
  const encryptTemporalSecret = (secret) => {
    const iv = crypto2.randomBytes(16);
    const cipher = crypto2.createCipheriv("aes-256-cbc", TOTP_MASTER_KEY, iv);
    let encrypted = cipher.update(secret, "utf8", "hex");
    encrypted += cipher.final("hex");
    return { iv: iv.toString("hex"), encrypted };
  };
  const decryptTemporalSecret = (encrypted, ivHex) => {
    const decipher = crypto2.createDecipheriv("aes-256-cbc", TOTP_MASTER_KEY, Buffer.from(ivHex, "hex"));
    let decrypted = decipher.update(encrypted, "hex", "utf8");
    decrypted += decipher.final("utf8");
    return decrypted;
  };
  const hashNode = (route) => {
    return crypto2.createHash("sha256").update(route.toLowerCase()).digest("hex");
  };
  const readTemporalStore = () => {
    try {
  ...
  app.post("/api/temporal-node/verify", (req, res) => {
    try {
      ...
      const currentEpoch = Math.floor(Date.now() / 1e3);
      const step = 30;
      ...
      for (const token of possibleTokens) {
        try {
          ...
          if (decryptedStr) {
            const payload = JSON.parse(decryptedStr);
            if (payload.action === "verify_temporal" && payload.timestamp && Date.now() - payload.timestamp < 5 * 60 * 1e3) {
              isZkpValid = true;
              break;
            }
          }
    ...
    try {
      ...
      for (const token of possibleTokens) {
        try {
          ...
          if (decryptedStr) {
            const payload = JSON.parse(decryptedStr);
            if (payload.action === "pack_export" && payload.timestamp && Date.now() - payload.timestamp < 5 * 60 * 1e3) {
              verified = true;
              break;
            }
          }
